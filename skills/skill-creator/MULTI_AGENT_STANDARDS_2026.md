...
4.  **Paso N+2 (Validation Protocol & Zero Omission)**: Implementar validación explícita antes de un "handoff" (ej. "Esta respuesta cumple el schema JSON?"). **Zero Omission**: Verificar que CADA skill listada en la cabecera `⬡ SKILLS ACTIVADAS` del handshake tenga su obligatoria y respectiva explicación en la sección `Applied`.
5.  **Paso N+3 (Output & Anti-Truncation)**: Emite la respuesta en Markdown o el JSON de orquestación. **Ley CREAM Estricta**: Está estrictamente DISUADIDO Y PENADO truncar, resumir o "amputar" ramas de código con comentarios (como `// resto del código`) para ahorrar tokens al momento de dar la respuesta; la Trazabilidad Total del código escrito debe ser del 100%.
